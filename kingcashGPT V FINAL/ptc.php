...
/*
echo
"<center><h1>This feature isnt available......yet<br><br><font color=\"green\">^___^ Nice try though!<br><a href=\"offers.php\">[Go To Offers Page]</a></font></h1></center></div>
<div id=\"right\">";
...
if ($userinfo['current_points_balance'] == 0.00) {
echo "<center><h1><b>You will need to earn some points so that you will have points to transfer -__- ......<a href=\"offers.php\">[Go To Offers Page]</a> </h1></center>";
...
if ($_POST['transfer']!='') {
	...
	if ($_POST['transfer']>$userinfo['current_points_balance']) {
		$error .= "<center><font color=\"red\"><b>You have less then $".htmlentities($_POST['transfer'])." in your balance.<br>Please enter in $".$userinfo['current_points_balance']." or less. </b></font></center>";
	}
}
if (($error=='') && ($_POST['transfer'])) {
	$transfer = mysql_real_escape_string($_POST['transfer']);
	$transfer2 = $transfer / 100;
	$q1 = "UPDATE users SET current_points_balance=current_points_balance-'".$transfer."' WHERE id='".$userinfo['id']."'";
	mysql_query($q1) or die(mysql_error());
	$q2 = "UPDATE users SET current_balance=current_balance+'".$transfer2."' WHERE id='".$userinfo['id']."'";
...
if (($userinfo['username'] == 'TinTurkeyz') || ($userinfo['username'] == 'admin')) {
echo"
