...
if(($request_ip == $adscend_ip ) || ($request_ip == $adscend_ip2 ) || ($request_ip == $adscend_ip3 )) {
	...
	$offerselect = mysql_fetch_assoc(mysql_query("SELECT * FROM offers WHERE cid='".$campaignid."'"));
	 ...
	 if($credit == 1){ //completed
	    ...
	    $users = mysql_fetch_assoc(mysql_query("SELECT * users WHERE username='".$yti."' LIMIT 1"));
	    ...
	    $pointsadd = "UPDATE `users` SET `current_points_balance` = `current_points_balance` + ".$rate." AND `total_points_earned` = `total_points_earned` + ".$rate." WHERE username = '".$yti."'";
	    mysql_query($pointsadd) or die(mysql_error()); // add points.   
	    $messagejoin = "<b><b style=\"color:green;\">".$yti."</b> earned ".$rate." points on <b>".$offerselect['name']."!</b>"; 
	    
	    } else if ($offerselect['reward_type'] = "Cash") { //cash offer
	    $cashadd = "UPDATE `users` SET `total_earned` = `total_earned` + ".$rate." AND `current_balance`= `current_balance` + ".$rate." WHERE username = '".$yti."'";
	    mysql_query($cashadd) or die(mysql_error()); // add cash. 
	    $messagejoin = "<b><b style=\"color:green;\">".$yti."</b> earned \$".$rate." on ".$offerselect['name']."!</b>";
	    }
	    ...
	    if ( $offerselect['reward_type'] = "Points") { //points offer
	    $rate = $rate * 100; //gets you accurate points
	    $pointssub = "UPDATE `users` SET `current_points_balance` = `current_points_balance` - ".$rate." AND `total_points_earned` = `total_points_earned` - ".$rate." WHERE username = '".$yti."'";
	    mysql_query($pointssub) or die(mysql_error()); // subtracts points.    
	    } else if ($offerselect['reward_type'] = "Cash") { //cash offer
	    $cashsub = "UPDATE `users` SET `total_earned` = `total_earned` - ".$rate." AND `current_balance`= `current_balance` - ".$rate." WHERE username = '".$yti."'";
