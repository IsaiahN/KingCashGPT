...
$operation_offers_ip = '174.120.146.43';
...
if ($request_ip == $operation_offers_ip ) {
	...
	$rate = $rate * .60;
	$offerselect = mysql_fetch_assoc(mysql_query("SELECT * FROM offers WHERE cid='888".$campaignid."'"));
	
	 if($credit == 1){ //completed
	    $date = date("F j, Y, g:i a");
	    $users = mysql_fetch_assoc(mysql_query("SELECT * FROM users WHERE username='".$yti."' LIMIT 1"));
	    $qcomplete = "INSERT INTO `completed`(`id`, `offer_id`, `user_id`, `date_submitted`, `reward`) VALUES ('','888".$campaignid."','".$users['id']."','".$date."','".$rate."')";
	    mysql_query($qcomplete) or die(mysql_error()); //puts offer into the completed list
	    
	    $delpending = "DELETE * FROM pending WHERE  user_id='".$users['id']."' AND offer_id='888".$campaignid."' ";
	    ...
	    $delcomplete = "DELETE * FROM completed WHERE  user_id='".$users['id']."' AND offer_id='888".$campaignid."' ";
	    mysql_query($delcomplete); // removes offer from the completed list.
	    $delpending = "DELETE * FROM pending WHERE  user_id='".$users['id']."' AND offer_id='888".$campaignid."' ";
	    mysql_query($delpending); // removes offer from the pending list.
	    $setdeny = "INSERT INTO `pending`(`id`, `offer_id`, `user_id`, `date_submitted`, `reward`, `status`) VALUES ('','888".$campaignid."','".$users['id']."','".$date."','".$rate."', '2')";
